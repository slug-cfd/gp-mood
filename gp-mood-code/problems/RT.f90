module parameters
   ...
   ! Mesh parameter
   integer,  parameter :: lf = 64 ! Number of cell in the x direction
   integer,  parameter :: nf = 3*64  ! Number of cell in the y direction

   ! IC, BC and domain setup
   integer, parameter  :: problem = RT
   real(PR), parameter :: tmax = 12.4
   integer, parameter  :: nmax = 99999 ! put a large number if want to finish based on tmax only
   real(16), parameter :: Lx_16 = 0.5 !Lenght of the domain in the x-direction
   real(16), parameter :: Ly_16 = 1.5 !Lenght of the domain in the y-direction
   integer, parameter  :: BC_type = RT_BC ! Boundary conditions
