...
class data2d:
    ...
    def __init__(self, file_name):  # 'self' means the class itself.
        ...
        self.rho = np.reshape(self.raw[:, 2], (self.xbins, self.ybins)).T
        self.mx  = np.reshape(self.raw[:, 3], (self.xbins, self.ybins)).T
        self.my  = np.reshape(self.raw[:, 4], (self.xbins, self.ybins)).T
        self.p   = np.reshape(self.raw[:, 5], (self.xbins, self.ybins)).T


# load data using a class
#d2 = data2d('final.dat')

baseName = 'implosion_1quad_rk3_3rdDivvDMP_400'
dataName = baseName+'_final.dat'
...
levels = np.linspace(0.3, 1.2, 40)   # this is a 1d array with 40 elements, ranging from 0.1 to 1.8.
